...
var (
	...
	Eval = map[string]func(int, int) int{
...
func init() {
	...
	SubElement = &Element{eType: SubOperation}
	SubElement.value.WriteByte('-')

	MultElement = &Element{eType: MultOperation}
	MultElement.value.WriteByte('*')

	DivElement = &Element{eType: DivOperation}
	DivElement.value.WriteByte('/')
...
func (e *Element) IsSubOperation() bool {
	return e.eType == SubOperation
}
...
func (e *Element) IsDivOperation() bool {
	return e.eType == DivOperation
}
